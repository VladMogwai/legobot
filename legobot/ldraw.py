...
def write_ldr(bricks: list[PlacedBrick], path: str, name: str, color: int) -> None:
    ...
    for _, layer_bricks in groupby(sorted(bricks, key=lambda b: b.layer), key=lambda b: b.layer):
        lines.extend(_brick_line(b, color) for b in layer_bricks)
...
def _brick_line(b: PlacedBrick, color: int) -> str:
    cx = (b.x + b.width / 2) * STUD_LDU
    cz = (b.z + b.length / 2) * STUD_LDU
    cy = -b.layer * BRICK_HEIGHT_LDU  # в LDraw вверх — это -Y
    rot = _ROTATE_90 if b.rotated else _IDENTITY
    return f"1 {color} {cx:.6f} {cy:.6f} {cz:.6f} {rot} {b.part.number}.dat"
